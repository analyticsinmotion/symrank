...
def benchmark_symrank_topk_batched(query: np.ndarray, candidates: np.ndarray, top_k: int, vector_size: int, batch_size: int):
...
def run():
    ...
    benchmarks = [
        ("SymRank_TopK", lambda: benchmark_symrank_topk_batched(query, candidates, top_k, dim, batch_size)),
        ("sklearn_TopK", lambda: benchmark_sklearn_topk_batched(query, candidates, top_k, batch_size)),
        ("NumPy_TopK", lambda: benchmark_numpy_topk_batched(query, candidates, top_k, batch_size)),
    ]
