...
def benchmark_sklearn(query: np.ndarray, candidates: np.ndarray, top_k: int):
    sims = skl_cosine(query.reshape(1, -1), candidates)[0]
    topk_idx = np.argsort(sims)[-top_k:][::-1]
    return [(i, sims[i]) for i in topk_idx]


def benchmark_numpy(query: np.ndarray, candidates: np.ndarray, top_k: int):
...
def benchmark_symrank(query: np.ndarray, candidates: np.ndarray, top_k: int):
...
def run():
    ...
    benchmarks = [
        ("SymRank", lambda: benchmark_symrank(query, candidates, top_k)),
        ("sklearn", lambda: benchmark_sklearn(query, candidates, top_k)),
        ("NumPy", lambda: benchmark_numpy(query, candidates, top_k)),
    ]
    ...
    for name, fn in benchmarks:
        ...
        print(f"{name:<10} | Time: {t1 - t0:.6f} seconds")
